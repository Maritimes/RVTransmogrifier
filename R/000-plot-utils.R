...
getBbox <- function(filterField = "StrataID", filterVals = NULL) {
  ...
  if (!is.null(filterField) && !is.null(filterVals)) {
    strata <- strata[strata[[filterField]] %in% filterVals, ]
  }
...
ggBathy <- function(plotBathy = "FILL", bathyIntervals = 100) {
  ...
  if (!is.null(plotBathy)) {
    ...
    if (plotBathy == "FILL") {
      ...
      ggItems[["bathy"]] <- c(
        ggItems[["bathy"]],
        ggplot2::scale_fill_manual(values = depths(length(bathyBreaks)))
      )
...
ggStrata <- function(plotStrata = TRUE, plotLabels = TRUE, filter = NULL) {
  ...
  strata <- sf::st_transform(strata, crs = 4326)
  if (!is.null(filter)) {
    strata <- strata[strata$StrataID %in% filter, ]
  }
  if (plotStrata) {
    ggItems[["strata"]] <- ggplot2::geom_sf(
      data = strata,
      fill = NA,
      color = "#00669933"
    )
...
ggCatchPts <- function(
  ...
  if (return == "CATCHES") {
    if (length(colourVar) > 1) {
      ...
      ggItems[["catchPts"]] <- suppressWarnings({
        ggplot2::geom_point(
          data = catchdata[!is.na(catchdata[, sizeVar]), ],
          ggplot2::aes_string(
            x = "SLONG_DD",
            y = "SLAT_DD",
            size = sizeVar,
      ...
      ggItems[["catchPts"]] <- suppressWarnings({
        ggplot2::geom_point(
          ...
          alpha = 0.6,
          show.legend = c(size = TRUE, colour = TRUE, shape = FALSE)
        )
      })
    }
    ggItems[["catchPts"]] <- c(
  ...
  if (return == "NULLSETS") {
    ...
    if (nrow(null_data) > 0) {
      ...
      ggItems[["nullSets"]] <- ggplot2::geom_point(
        data = null_data,
        ggplot2::aes(SLONG_DD, SLAT_DD, shape = "Null Set"),
        size = 2,
        color = "black"
      )
      ggItems[["nullSets"]] <- c(
...
ggStrataData <- function(
  ...
  ggItems <- list()
  ggItems[["ggStratData"]] <- ggplot2::geom_sf(
    data = catchStrataData,
    ggplot2::aes_string(fill = plotField)
  )
  ggItems[["ggStratData"]] <- c(
    ggItems[["ggStratData"]],
    ggplot2::scale_fill_continuous(
      name = paste0(plotField, "\n(", plotLabel, ")"),
      direction = -1,
      type = "viridis",
      na.value = NA
    )
