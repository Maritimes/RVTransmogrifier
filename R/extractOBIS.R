...
extractOBIS <- function(
  ...
  dataInfo <- function(thisSurv = NULL) {
    infoBlock <- data.frame(
      rightsHolder = 'His Majesty the King in right of Canada, as represented by the Minister of Fisheries and Oceans',
  ...
  doOBIS <- function(data = NULL, thisSurv = NULL, thisYear = NULL, debug = F) {
    taxonomicInfo <- function(data = NULL, SPEC = NULL, ...) {
      species_taxa <- data[
        data$CODE %in% SPEC & !is.na(data$APHIA_ID),
        c(
          "SPEC", #"scientificName"
          "COMM", #"vernacularName"
          "CODE", #"SPEC"
      ...
      species_taxa$scientificNameID <- paste0(
        "urn:lsid:marinespecies.org:taxname:",
        species_taxa$APHIA_ID
      )
      species_taxa$specificEpithet <- sub(
        '^.* ([[:alnum:]]+)$',
        '\\1',
        species_taxa$SPEC
      )
      species_taxa[
        which(species_taxa$SPEC == species_taxa$specificEpithet),
        "specificEpithet"
      ] <- NA
      species_taxa$scientificNameAuthorship <- NA
      colnames(species_taxa)[
        colnames(species_taxa) == "SPEC"
      ] <- "scientificName"
      colnames(species_taxa)[
        colnames(species_taxa) == "COMM"
      ] <- "vernacularName"
    ...
    setsPreProcessed <- data$GSINF |>
      dplyr::mutate(
        eventDate = lubridate::with_tz(SDATE, "America/Halifax"),
    ...
    catPreProcessed <- data$GSCAT |>
      ...
      as.data.frame()
    catPreProcessed <- merge(
      catPreProcessed,
      setsPreProcessed[, c("theSet", "DIST")]
    )

    catPreProcessed <- catPreProcessed |>
      dplyr::rename(TOTNO_RAW = TOTNO, TOTWGT_RAW = TOTWGT) |>
      dplyr::mutate(
        TOTWGT = TOTWGT_RAW * (1.75 / DIST),
    ...
    missionWKT <- setsPreProcessed[, c("id", "SLAT_DD", "SLONG_DD")] |>
      ...
      dplyr::group_by(id) |>
      dplyr::summarize(
        footprintWKT = sf::st_as_text(
          sf::st_as_sfc(sf::st_bbox(geometry)),
          crs = 4326
        )
      ) |>
      sf::st_drop_geometry() |>
    ...
    missionEvents <- setsPreProcessed |>
      dplyr::mutate(
        type = "Mission level information",
        eventID = id,
        parentEventID = NA,
      ...
      dplyr::summarize(
        .groups = "keep",
        eventDate = min(eventDate, na.rm = TRUE),
    ...
    # missionEvents$decimalLatitude = sf::::wkt_centroid(missionEvents$footprintWKT)[[2]]

    missionEvents$footprintWKT_sf <- sf::st_as_sfc(missionEvents$footprintWKT)
    missionEvents$centroid <- sf::st_centroid(missionEvents$footprintWKT_sf)
    missionEvents$decimalLongitude <- sf::st_coordinates(
      missionEvents$centroid
    )[, "X"]
    missionEvents$decimalLatitude <- sf::st_coordinates(
      missionEvents$centroid
    )[, "Y"]
    ...
    missionEMOF$measurementValue <- ifelse(
      missionEMOF$measurementValue == "CAB",
      "JOHN CABOT",
      ifelse(
        missionEMOF$measurementValue == "CAR",
        "JACQUES CARTIER",
        ifelse(
          missionEMOF$measurementValue == "VEN",
          "MERSEY VENTURE",
          ifelse(
            missionEMOF$measurementValue == "NED",
            "ALFRED NEEDLER",
            ifelse(
              missionEMOF$measurementValue == "TEL",
              "TELEOST",
              ifelse(
                missionEMOF$measurementValue == "TEM",
                "W. TEMPLEMAN",
                ifelse(
                  missionEMOF$measurementValue == "ATC",
                  "A. T. Cameron",
                  ifelse(
                    missionEMOF$measurementValue == "HAM",
                    "LADY HAMMOND",
                    "ERROR"
                  )
                )
    ...
    gearDets[
      gearDets$GEAR == 15,
      "GEARDESC"
    ] <- "US 4 seam 3 bridle survey trawl"
    ...
    setEvents <- setsPreProcessed |>
      ...
      dplyr::mutate(
        type = "Trawl level information",
        id = theSet,
        eventID = theSet,
        eventRemarks = paste0(
          "Trawl ",
          substr(eventID, nchar(eventID) - 2, nchar(eventID))
        ),
        locationRemarks = paste0(
          "Strata: ",
          STRAT,
          "; decimalLatitude and decimalLongitude are the start positions of each set"
        ),
        HOWD = HOWD,
    ...
    gearEMOF <- gearDets[, c("id", "gearType", "GEARDESC", "WSPREAD")] |>
      ...
      as.data.frame()
    gearEMOF[
      gearEMOF$measurementValue %in% gearDets$GEARDESC,
      c("measurementType", "measurementUnit", "measurementRemarks")
    ] <- as.data.frame(t(as.matrix(c(
      "trawl name",
      NA,
      "Name of fishing gear deployed"
    ))))
    gearEMOF[
      gearEMOF$measurementValue %in% gearDets$gearType,
      c("measurementType", "measurementUnit", "measurementRemarks")
    ] <- as.data.frame(t(as.matrix(c(
      "Sampling protocol",
      NA,
      "Type of fishing gear deployed"
    ))))
    gearEMOF[
      gearEMOF$measurementValue %in% gearDets$WSPREAD,
      c("measurementType", "measurementUnit", "measurementRemarks")
    ] <- as.data.frame(t(as.matrix(c(
      "trawl wingspread",
      "metres",
      "the horizontal distance between the wings"
    ))))

    setsPreProcessed <- merge(setsPreProcessed, setArea)

    setEMOF <- setsPreProcessed[,
      !names(setsPreProcessed) %in%
        c(
          "eventDate",
          "id",
          "AREA",
    ...
    setEMOF[
      setEMOF$measurementType == "DMAX_M",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c(
      "maximum (deepest) bottom depth observed during a tow",
      "metres"
    ))))
    setEMOF[
      setEMOF$measurementType == "DMIN_M",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c(
      "minimum (shallowest) bottom depth observed during a tow",
      "metres"
    ))))
    setEMOF[
      setEMOF$measurementType == "DUR",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("sample duration", "minutes"))))
    setEMOF[
      setEMOF$measurementType == "SPEED",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("average tow speed", "knots"))))
    ...
    catchBiomassEmof <- catPreProcessed |>
      dplyr::mutate(
        ...
        measurementRemarksSupplemental = ifelse(
          TOTWGT == 0,
          "Small catches can appear as 0 (they were rounded to the nearest kg)",
          NA
        )
      ) |>
      dplyr::select(
        -c("TOTNO_RAW", "TOTWGT_RAW", "TOTNO", "TOTWGT", "theSet", "theCatch")
      ) |>
      as.data.frame()
    ...
    catchEMOF$measurementRemarks <- apply(
      catchEMOF[, c("measurementRemarks", "measurementRemarksSupplemental")],
      1,
      function(i) {
        paste(stats::na.omit(i), collapse = "; ")
      }
    )
    ...
    detEMOF <- detPreProcessed[, !names(detPreProcessed) %in% c("AGMAT")] |>
      ...
      dplyr::mutate(
        emofID = paste(
          theCatch,
          formatC(detID, width = detPad, flag = 0),
          sep = ":"
        )
      ) |>
      dplyr::select(
        -c("theSet", "FSHNO", "SPECIMEN_ID", "theCatch", "detID")
      ) |>
      as.data.frame()
    detEMOF[
      detEMOF$measurement == "FWT",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("round weight of the specimen", "grams"))))
    detEMOF[
      detEMOF$measurement == "AGE",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("age", "years"))))
    detEMOF[
      detEMOF$measurement == "FSEX",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("sex", NA))))
    detEMOF[
      detEMOF$measurement == "FSEX" & detEMOF$measurementValue == 3,
      "measurementValue"
    ] <- "Female (berried)"
    detEMOF[
      detEMOF$measurement == "FSEX" & detEMOF$measurementValue == 2,
      "measurementValue"
    ] <- "Female"
    detEMOF[
      detEMOF$measurement == "FSEX" & detEMOF$measurementValue == 1,
      "measurementValue"
    ] <- "Male"
    detEMOF[
      detEMOF$measurement == "FSEX" & detEMOF$measurementValue == 0,
      "measurementValue"
    ] <- "Unknown"
    detEMOF[
      detEMOF$measurement == "FMAT",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("maturity", NA))))
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 0,
      "measurementValue"
    ] <- "Undetermined"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 1,
      "measurementValue"
    ] <- "Immature"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 2,
      "measurementValue"
    ] <- "Ripening 1"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 3,
      "measurementValue"
    ] <- "Ripening 2"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 4,
      "measurementValue"
    ] <- "Ripe (Mature)"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 5,
      "measurementValue"
    ] <- "Spawning (Running)"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 6,
      "measurementValue"
    ] <- "Spent"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 7,
      "measurementValue"
    ] <- "Recovering"
    detEMOF[
      detEMOF$measurement == "FMAT" & detEMOF$measurementValue == 8,
      "measurementValue"
    ] <- "Resting"

    #add a default values for FLEN that we'll overwrite for specific species
    detEMOF[
      detEMOF$measurement == "FLEN",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("Length (fork length)", "centimeters"))))
    # scallops
    detEMOF[
      detEMOF$SPEC %in%
        c(4320, 4321, 4322, 4324, 4325) &
        detEMOF$measurement == "FLEN",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("shell height", "millimeters"))))
    # squid
    detEMOF[
      detEMOF$SPEC %in%
        c(4511, 4512, 4514, 4664) &
        detEMOF$measurement == "FLEN",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("mantle length", "centimeters"))))
    # lobsters
    detEMOF[
      detEMOF$SPEC %in% c(2550:2553, 8145) & detEMOF$measurement == "FLEN",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("carapace length", "millimeters"))))
    #crabs
    detEMOF[
      detEMOF$SPEC %in% c(2506:2547, 6006) & detEMOF$measurement == "FLEN",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("carapace length", "millimeters"))))
    #herring
    detEMOF[
      detEMOF$SPEC %in% c(60) & detEMOF$measurement == "FLEN",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("Length (fork length)", "millimeters"))))
    detEMOF[
      detEMOF$SPEC %in% c(60) & detEMOF$measurement == "FLEN",
      c("measurementRemarks")
    ] <- "Starting in summer 2016, herring length is measured in mm. Prior measurements have been converted from cm."
    #grenadiers
    detEMOF[
      detEMOF$SPEC %in%
        c(409:414, 416, 1269, 1270, 998) &
        detEMOF$measurement == "FLEN",
      c("measurementType", "measurementUnit")
    ] <- as.data.frame(t(as.matrix(c("pre-anal fin length", "centimeters"))))
    detEMOF[
      detEMOF$SPEC %in%
        c(409:414, 416, 1269, 1270, 998) &
        detEMOF$measurement == "FLEN",
      c("measurementRemarks")
    ] <- "Some grenadier species are now measured using pre-anal instead of fork-length."
    ...
    sppOccurrence <- catPreProcessed |>
      ...
      dplyr::mutate(
        SPEC = as.numeric(substr(occurrenceID, 16, 19)),
    ...
    species_taxa <- taxonomicInfo(
      data = this$GSSPECIES,
      SPEC = unique(sppOccurrence$SPEC)
    )

    occurrence <- merge(
      sppOccurrence,
      species_taxa[, !names(species_taxa) %in% c("AphiaID", "SPEC")]
    )
    ...
    eventCore$footprintWKT_sf <- NULL
    ...
    EMOF <- rbind.data.frame(
      missionEMOF[, emof_fields],
      gearEMOF[, emof_fields]
    )
    EMOF <- rbind.data.frame(EMOF, setEMOF[, emof_fields])
    EMOF <- rbind.data.frame(EMOF, catchEMOF[, emof_fields])
    EMOF <- rbind.data.frame(EMOF, detEMOF[, emof_fields])
    ...
    utils::write.csv(
      results$eventCore,
      file = file.path(path, paste0(thisYear, "_", thisSurv, "_eventCore.csv")),
      row.names = F
    )
    utils::write.csv(
      results$occurrence,
      file = file.path(
        path,
        paste0(thisYear, "_", thisSurv, "_occurrence.csv")
      ),
      row.names = F
    )
    utils::write.csv(
      results$EMOF,
      file = file.path(path, paste0(thisYear, "_", thisSurv, "_EMOF.csv")),
      row.names = F
    )
  ...
  for (s in 1:length(survey)) {
    for (y in 1:length(years)) {
      ...
      obis <- doOBIS(this, thisSurv = survey[s], thisYear = years[y])
      results[[name]] <- obis
      obis <- NULL
    }
  }
