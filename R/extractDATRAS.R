...
extractDATRAS <- function(
  ...
  addLenMeasInfo <- function(df = NULL) {
    ...
    df[
      (df$SPEC >= 2506 & df$SPEC <= 2547) | df$SPEC == 6006,
      c("LENMEASTYPE", "LNGTCODE")
    ] <- data.frame(7, 0.1)
    #lobsters -  6 "Carapace Length"; mm
    df[
      df$SPEC %in% c(2550, 2551, 2552, 2553, 8145),
      c("LENMEASTYPE", "LNGTCODE")
    ] <- data.frame(6, 0.1)
    #scallops -  9 "Shell Height"; mm
    df[
      df$SPEC %in% c(4320, 4321, 4322, 4324, 4325),
      c("LENMEASTYPE", "LNGTCODE")
    ] <- data.frame(9, 0.1)
  ...
  Mar_HH <- function(scratch_env = NULL, survey = NULL) {
    ...
    df = merge(
      scratch_env$GSINF,
      scratch_env$GSMISSIONS,
      all.x = T,
      by = c("MISSION")
    )
    df = merge(
      df,
      unique(scratch_env$GSWARPOUT),
      all.x = T,
      by = c("MISSION", "SETNO")
    )
    ...
    calcValues <- function(df = NULL) {
      ...
      processTimes <- function(df = NULL) {
        ...
        df[is.na(df$DATETIME), "DATETIME"] <- lubridate::make_datetime(
          year = df[is.na(df$DATETIME), "YEAR"],
          month = df[is.na(df$DATETIME), "MONTH"],
          day = df[is.na(df$DATETIME), "DAY"],
          hour = df[is.na(df$DATETIME), "HOUR"] + 1,
          min = df[is.na(df$DATETIME), "MIN"],
          sec = 0,
          tz = "Canada/Atlantic"
        )
        ...
        df <- df |>
          dplyr::rowwise() |>
          dplyr::mutate(
            tmp = list(suncalc::getSunlightTimes(
              date = as.Date(DATETIME),
              lat = slat_dd,
              lon = slong_dd
            )),
            DAYNIGHT = ifelse(
              DATETIME > tmp$sunrise & DATETIME < tmp$sunset,
              "D",
              "N"
            )
          ) |>
      ...
      processCurrents <- function(df = NULL) {
          ...
          ifelse(
            ...
            ifelse(
              ...
              ifelse(
                df$curnt == 4,
                180,
                ifelse(df$curnt == 5, 999, ifelse(df$curnt == 6, NA, NA))
              )
            )
          )
        )
        df[
          !is.na(df$TOWDIR) & (!is.na(df$SURCURDIR) & df$SURCURDIR < 999),
          "SURCURDIR"
        ] <-
          df[
            !is.na(df$TOWDIR) & (!is.na(df$SURCURDIR) & df$SURCURDIR < 999),
        ...
        df[!is.na(df$SURCURDIR) & df$SURCURDIR < 0, "SURCURDIR"] <- df[
          !is.na(df$SURCURDIR) & df$SURCURDIR < 0,
          "SURCURDIR"
        ] +
          360
        df[
          !is.na(df$SURCURDIR) & df$SURCURDIR > 360 & df$SURCURDIR < 999,
          "SURCURDIR"
        ] <- df[
          !is.na(df$SURCURDIR) & df$SURCURDIR > 360 & df$SURCURDIR < 999,
          "SURCURDIR"
                ...
                ifelse(
                  ...
                  ifelse(
                    ...
                    ifelse(
                      df$force == 6,
                      22,
                      ifelse(df$force == 7, 28, ifelse(df$force == 8, 34, NA))
                    )
                  )
                )
              )
    ...
    addPlatformDets <- function(df = NULL) {
      ...
      addGearDets <- function(df = NULL) {
        ...
        df[, c("KITEDIM", "TICKLER", "GEAREXP", "RIGGING")] <- data.frame(
          -9,
          -9,
          -9,
          "BM"
        ...
        if (nrow(df[df$gear %in% 9, ]) > 0) {
          ...
          ] <- data.frame("W2A", 12.5, 42.3672, 4, 950, 4.57, "PE")
        } #Doortype should be "PORTUGUESE"
        # year-specific changes for W2A -------------------------------------------

        if (nrow(df[df$gear %in% 9 & df$YEAR >= 1982 & df$YEAR < 2007, ]) > 0) {
          df[
            df$gear %in% 9 & (df$YEAR >= 1982 & df$YEAR < 2007),
            c("WARPDIA")
          ] <- 28.6
        }
        if (nrow(df[df$gear %in% 9 & df$YEAR >= 2007 & df$YEAR < 2012, ]) > 0) {
          df[
            df$gear %in% 9 & (df$YEAR >= 2007 & df$YEAR < 2012),
            c("WARPDIA", "WARPDEN", "WGTGROUNDROPE")
          ] <- data.frame(25.4, 3.17, 52.6694)
        }
        if (nrow(df[df$gear %in% 9 & df$YEAR < 2007, ]) > 0) {
          df[
            df$gear %in% 9 & df$YEAR < 2007,
            c("SWEEPLNGT", "BUOYANCY")
          ] <- data.frame(round(37.7952, 0), round(195.16, 0))
        }
        if (nrow(df[df$gear %in% 9 & df$YEAR >= 2007, ]) > 0) {
          df[
            df$gear %in% 9 & df$YEAR >= 2007,
            c("SWEEPLNGT", "BUOYANCY")
          ] <- data.frame(round(33.8328, 0), round(199.68))
        }

        # Yankee 36 Constants -----------------------------------------------------
        if (nrow(df[df$gear %in% 3, ]) > 0) {
          df[
            df$gear %in% 3,
            c(
              "GEAR",
        ...
        } #Doortype should be "PolyIceOval"
        if (nrow(df) > 0) {
          df$gear <- NULL
        } #by the time we get here, we can delete this field
        return(df)
      }
    ...
    getDepths <- function(df = NULL) {
      ...
      if (length(df[!is.na(df$depth), "depth"]) > 0) {
        df[!is.na(df$depth), "DEPTH"] <- df[!is.na(df$depth), "depth"]
      }
    ...
    convertUnits <- function(df = NULL) {
      ...
      if (length(df[!is.na(df$warpout), "warpout"]) > 0) {
        df[!is.na(df$warpout), "warpout"] <- round(
          df[!is.na(df$warpout), "warpout"] * 1.8288,
          0
        )
      } #depth from fathoms to meters (non NA)
      if (length(df[!is.na(df$dist), "dist"]) > 0) {
        df[!is.na(df$dist), "dist"] <- round(
          df[!is.na(df$dist), "dist"] * 1852,
          0
        )
      } #distance from NM to meters (non NA)
      if (length(df[!is.na(df$speed), "speed"]) > 0) {
      ...
      if (length(df[!is.na(df$DEPTH), "DEPTH"]) > 0) {
        df[!is.na(df$DEPTH), "DEPTH"] <- round(
          df[!is.na(df$DEPTH), "DEPTH"] * 1.8288,
          0
        )
      }
  ...
  Mar_HL <- function(scratch_env = NULL) {
    ...
    handleGSCAT <- function() {
      ...
      doSubFact <- function(df = NULL) {
        ...
        df$SUBFACTOR <- -9
        df[df$TOTWGT > 0 & df$SAMPWGT > 0, "SUBFACTOR"] <- df[
          df$TOTWGT > 0 & df$SAMPWGT > 0,
          "TOTWGT"
        ] /
          df[df$TOTWGT > 0 & df$SAMPWGT > 0, "SAMPWGT"]
      ...
      if (nrow(df[df$TOTWGT == df$SUBWGT, ]) > 0) {
        df[df$TOTWGT == df$SUBWGT, "SUBWGT"] <- -9
      }
      if (length(df[which(df$SUBWGT == 0), "SUBWGT"]) > 0) {
        df[which(df$SUBWGT == 0), "SUBWGT"] <- -9
      }
    ...
    handleGSDET <- function() {
      ...
      handleDetSpecies <- function(df = NULL) {
        ...
        if (
          nrow(df[
            which(
              df$SPEC == 60 &
                (substr(df$MISSION, 4, 7) <= 2016 &
                  (!df$MISSION %in% c("NED2016016")))
            ),
          ]) >
            0
        ) {
      ...
      df <- scratch_env$GSDET[,
        names(scratch_env$GSDET) %in%
          c(
            "MISSION",
            "SETNO",
            "SPEC",
    ...
    handleSpecies <- function() {
      ...
      SPP <- sort(unique(c(
        unique(scratch_env$GSCAT$SPEC),
        unique(scratch_env$GSDET$SPEC)
      )))
    ...
    tmp_HL3[
      which(tmp_HL3$CATCATCHWGT == -9 & tmp_HL3$TOTALNO == 9),
      "SPECVAL"
    ] <- 5
  ...
  Mar_CA <- function(scratch_env = NULL) {
    ...
    handleGSDET <- function() {
      ...
      addMaturity <- function(df = NULL) {
        df$MATURITYSCALE <- "M6"
        df$MATURITY <- -9
        if (nrow(df[which(is.na(df$FMAT)), ]) > 0) {
          df[which(is.na(df$FMAT)), "MATURITY"] <- -9
        }
        if (nrow(df[which(df$FMAT == 0), ]) > 0) {
          df[which(df$FMAT == 0), "MATURITY"] <- -9
        } #DClark indicates that 0 should be NULL
        if (nrow(df[which(df$FMAT == 9), ]) > 0) {
          df[which(df$FMAT == 9), "MATURITY"] <- -9
        } #DClark indicates that 9 is probably an error
        if (nrow(df[which(df$FMAT %in% c(1)), ]) > 0) {
          df[which(df$FMAT %in% c(1)), "MATURITY"] <- 61
        }
        if (nrow(df[which(df$FMAT %in% c(2, 3, 4)), ]) > 0) {
          df[which(df$FMAT %in% c(2, 3, 4)), "MATURITY"] <- 62
        }
        if (nrow(df[which(df$FMAT %in% c(5)), ]) > 0) {
          df[which(df$FMAT %in% c(5)), "MATURITY"] <- 63
        }
        if (nrow(df[which(df$FMAT %in% c(6)), ]) > 0) {
          df[which(df$FMAT %in% c(6)), "MATURITY"] <- 64
        }
        if (nrow(df[which(df$FMAT %in% c(7, 8)), ]) > 0) {
          df[which(df$FMAT %in% c(7, 8)), "DEVSTAGE"] <- 65
        }
        df$FMAT <- NULL
        return(df)
      }
      addAges <- function(df = NULL) {
        ...
        df$AGESOURCE <- -9
        if (nrow(df[which(df$AGMAT == 1), ]) > 0) {
          df[which(df$AGMAT == 1), "AGESOURCE"] <- "OT"
        }
        if (nrow(df[which(df$AGMAT == 2), ]) > 0) {
          df[which(df$AGMAT == 2), "AGESOURCE"] <- "SC"
        }
        if (nrow(df[which(df$AGMAT == 4), ]) > 0) {
          df[which(df$AGMAT == 4), "AGESOURCE"] <- "VR"
        }
        ...
        if (
          nrow(df[
            which(
              df$SPEC == 60 &
                (substr(df$MISSION, 4, 7) < 2016 |
                  (df$MISSION %in% c("TEL2016002", "TEL2016003")))
            ),
          ]) >
            0
        ) {
      ...
      df <- addAges(df)
      df <- handleDetSpecies(df)
      if (nrow(df[which(is.na(df$FWT)), ]) > 0) {
        df[which(is.na(df$FWT)), "FWT"] <- 0
      }
      df$FMAT <- NULL
      return(df)
    }
    handleSpecimens <- function() {
      df <- scratch_env$GS_LV1_OBSERVATIONS[
        paste0(
          scratch_env$GS_LV1_OBSERVATIONS$MISSION,
          "_",
          scratch_env$GS_LV1_OBSERVATIONS$SETNO,
          "_",
          scratch_env$GS_LV1_OBSERVATIONS$SPEC,
          "_",
          scratch_env$GS_LV1_OBSERVATIONS$SIZE_CLASS
        ) %in%
          paste0(
            GSDET$MISSION,
            "_",
      ...
      if (
        nrow(df[
          grep(pattern = "Genetic", x = df$LV1_OBSERVATION, ignore.case = T),
        ]) >
          0
      ) {
        df[
      ...
      if (
        nrow(df[
          grep(
            pattern = "Stomach Number",
            x = df$LV1_OBSERVATION,
            ignore.case = T
          ),
        ]) >
          0
    ...
    if (is.null(GSspecimens)) {
      ...
      GSDET <- merge(
        GSDET,
        unique(GSspecimens[
          GSspecimens$GENSAMP == "Y",
          c("MISSION", "SETNO", "SPEC", "SIZE_CLASS", "SPECIMEN_ID", "GENSAMP")
        ]),
        all.x = T,
        by = c("MISSION", "SETNO", "SPEC", "SIZE_CLASS", "SPECIMEN_ID")
      )
      GSDET <- merge(
        GSDET,
        unique(GSspecimens[
          GSspecimens$STOMSAMP == "Y",
          c("MISSION", "SETNO", "SPEC", "SIZE_CLASS", "SPECIMEN_ID", "STOMSAMP")
        ]),
        all.x = T,
        by = c("MISSION", "SETNO", "SPEC", "SIZE_CLASS", "SPECIMEN_ID")
      )
      GSDET <- merge(
        GSDET,
        unique(GSspecimens[
          GSspecimens$PARSAMP == "Y",
          c("MISSION", "SETNO", "SPEC", "SIZE_CLASS", "SPECIMEN_ID", "PARSAMP")
        ]),
        all.x = T,
        by = c("MISSION", "SETNO", "SPEC", "SIZE_CLASS", "SPECIMEN_ID")
      )
    ...
    if (length(forAgg[is.na(forAgg$FWT), "FWT"]) > 0) {
      forAgg[is.na(forAgg$FWT), "FWT"] <- 0
    }
    ...
    if (length(forAgg_indwgt[forAgg_indwgt$INDWGT == 0, "INDWGT"]) > 0) {
      forAgg_indwgt[forAgg_indwgt$INDWGT == 0, "INDWGT"] <- -9
    }
  ...
  for (y in 1:length(years)) {
    for (s in 1:length(survey)) {
      ...
      tmp = new.env()
      Mar.utils::get_data_tables(
        schema = "GROUNDFISH",
        cxn = cxn,
        data.dir = get_pesd_rvt_dir(),
        tables = "GS_LV1_OBSERVATIONS",
        env = tmp
      )
      tmp_env$GS_LV1_OBSERVATIONS <- tmp$GS_LV1_OBSERVATIONS

      tmp_env$GS_LV1_OBSERVATIONS <- tmp_env$GS_LV1_OBSERVATIONS[
        tmp_env$GS_LV1_OBSERVATIONS$MISSION %in%
          tmp_env$GSMISSIONS$MISSION &
      ...
      if (nrow(tmp_env$GSINF) == 0) {
        ...
        utils::write.csv(
          x = NA,
          file = file.path(path, paste0(fullnm, "_noResults.csv")),
          row.names = F
        )
        next
      }
      ...
      SPP <- sort(unique(c(
        unique(tmp_env$GSCAT$SPEC),
        unique(tmp_env$GSDET$SPEC)
      )))
      SPP <- data.frame(SPEC = SPP)
      GSSPECIES_CODES <- tmp_env$GSSPECIES_NEW[
        tmp_env$GSSPECIES_NEW$CODE %in% SPP$SPEC,
        c("CODE", "APHIA_ID")
      ]

      colnames(GSSPECIES_CODES)[
        colnames(GSSPECIES_CODES) == "APHIA_ID"
      ] <- "SPECCODE"
      ...
      if (debug) {
        ...
        tmp_CA <- tmp_CA[
          tmp_CA$STNO == CAspMAX$STNO & tmp_CA$SPECCODE == CAspMAX$SPECCODE,
        ]
        tmp_HL <- tmp_HL[
          tmp_HL$STNO == CAspMAX$STNO & tmp_HL$SPECCODE == CAspMAX$SPECCODE,
        ]
      ...
      for (s in 1:length(SHIPS)) {
        ...
        if (csv) {
          ...
          suppressWarnings(utils::write.table(
            x = this_tmp_HH,
            file = file.path(path, fullnmShip),
            row.names = F,
            col.names = T,
            quote = FALSE,
            sep = ","
          ...
          if (debug) {
            utils::write.csv(
              x = this_tmp_HH,
              file = paste0(
                gsub(pattern = ".csv", replacement = "", x = fullnmShip),
                "_HH_debug.csv"
              ),
              row.names = F
            )
