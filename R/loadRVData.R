...
loadRVData <- function(cxn=NULL, force.extract = FALSE, ...){
  ...
  dataAllExists <- all(file.exists(file.path(get_pesd_rvt_dir(), paste0("GROUNDFISH.", coreTables, ".RData"))))
  
  if (dataAllExists && !force.extract){
    message("Found existing data. Loading...")
    #just load what we have
    result <- invisible(Mar.utils::get_data_tables(schema = "GROUNDFISH", tables = coreTables, 
                              cxn = cxn, data.dir = get_pesd_rvt_dir(), extract_user= args$extract_user, extract_computer = args$extract_computer,env = newE))
    
  }else{
    #do extraction, process and load
    message("Initial data extraction/processing - this will take a minute (subsequent loads will be WAY faster)")
    result <-     invisible(Mar.utils::get_data_tables(schema = "GROUNDFISH", tables = rawTables, 
                                         cxn = cxn, data.dir = get_pesd_rvt_dir(), extract_user= args$extract_user, extract_computer = args$extract_computer,env = newE))
    ...
    #make GSDETProducts
    newE$GSDETProducts <- disentangleGSDET(newE$GSDET, GSCAT_ = newE$GSCAT, GSINF_ = newE$GSINF)
    newE$GSDET_LF  <- newE$GSDETProducts$GSDET_LF
    newE$GSDET_DETS <- newE$GSDETProducts$GSDET_DETS
    
    #size classes removed from GSCAT
    newE$GSCAT <- rmSizeClasses(newE$GSCAT)
    ...
    assign("newGSSPECIES", newE$GSSPECIES_NEW, envir = .GlobalEnv)
    
    rm(list = c("GSDETProducts", "GSSPEC", "GSSPECIES_ANDES", "GSSPECIES_CHANGES_", "GSSPECIES", "GSSPECIES_CHANGES"), envir = newE)
