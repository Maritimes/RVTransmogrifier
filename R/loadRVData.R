...
loadRVData <- function(cxn = NULL, force.extract = FALSE, stock = NULL, ...) {
  ...
  if (is.null(args$keep_nullsets)) {
    args$keep_nullsets <- TRUE
  }
  ...
  if (!is.null(stock)) {
    if (!exists("stock_map", where = .GlobalEnv)) {
      stop(
        "The stock_map object is not loaded. Ensure `stock-utils.R` is sourced."
      )
    }
  ...
  missingTables <- coreTables[
    !file.exists(file.path(
      get_pesd_rvt_dir(),
      paste0("GROUNDFISH.", coreTables, ".RData")
    ))
  ]

  if (length(missingTables) == 0 & force.extract == F) {
    # no missing tables, load everything
    invisible(Mar.utils::get_data_tables(
      schema = "GROUNDFISH",
      tables = coreTables,
      force.extract = force.extract,
      cxn = cxn,
      data.dir = get_pesd_rvt_dir(),
      extract_user = args$extract_user,
      extract_computer = args$extract_computer,
      env = newE
    ))
  } else {
    if (force.extract) {
      ...
      message(
        "Initial data extraction/processing - this will take a minute (subsequent loads will be WAY faster)"
      )
      invisible(Mar.utils::get_data_tables(
        schema = "GROUNDFISH",
        tables = coreTables,
      ...
      message(
        "Extracting and processing missing tables - this will take a minute (subsequent loads will be WAY faster)"
      )
    ...
    if ("GSINF" %in% missingTables) {
      invisible(Mar.utils::get_data_tables(
        schema = "GROUNDFISH",
        tables = c("GSINF", "GSSTRATA"),
        force.extract = force.extract,
        cxn = cxn,
        data.dir = get_pesd_rvt_dir(),
        extract_user = args$extract_user,
    ...
    if ("GSINF" %in% missingTables | force.extract) {
      message(
        "Tweaking GSINF (adding dd coords, wingspreads and strata areas in km2"
      )
      ...
      newE$GSINF <- newE$GSINF |>
        dplyr::left_join(gearDets, by = "GEAR") |>
        dplyr::left_join(
          newE$GSSTRATUM |>
            dplyr::select(STRAT, AREA_KM2 = AREA) |>
            dplyr::mutate(AREA_KM2 = sqNMToSqKm(AREA_KM2)),
          by = "STRAT"
        )
    }
    if ("GSDET" %in% missingTables) {
      invisible(Mar.utils::get_data_tables(
        schema = "GROUNDFISH",
        tables = c("GSDET"),
        force.extract = force.extract,
    ...
    if ("GSDET" %in% missingTables | force.extract) {
      message(
        "Tweaking GSDET (converting herring to millimeters  and fixing grenadier fork lengths)"
      )
  ...
  if (as.integer(difftime(curDate, dataDate)) > 270) {
    message(
      "Your most recent data is >9 months old - there's probably new data available.\nConsider updating your data by adding `force.extract=T`. "
    )
  }

  df_names <- ls(envir = newE)[sapply(ls(envir = newE), function(x) {
    is.data.frame(get(x, envir = newE, inherits = FALSE))
  })]
  tblList <- stats::setNames(
    lapply(df_names, get, envir = newE, inherits = FALSE),
    df_names
  )
