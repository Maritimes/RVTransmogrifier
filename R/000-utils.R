...
#extractOpenData
utils::globalVariables(c("NAFO_AREA_ID", "STRAT_AREA_KM2", "MATURITY", "SEX"))
...
getSpInfo <- function(taxa = NULL, tblList) {
  ...
  these <- tblList$GSSPECIES_NEW[
    which(apply(
      tblList$GSSPECIES_NEW[, c(
        "COMM",
        "SPEC",
        "KINGDOM",
        "PHYLUM",
      ...
      1,
      function(r) any(grepl(taxa, toupper(r)))
    )),
    retFields
  ]
  these <- these |> unique()
...
fixGrenadierLengths <- function(GSDET_ = NULL) {
  ...
  GSDET_ |>
    dplyr::mutate(
      FLEN = dplyr::case_when(
        SPEC == 410 &
          substr(MISSION, 4, 7) < 2019 &
          MISSION != "NED2010027" ~ ((FLEN - 1.770067) / 4.453725) * 10,
        SPEC == 410 &
          MISSION == "NED2010027" &
          !SETNO %in% c(219, 222, 223, 229, 230, 233, 236) ~ ((FLEN -
          1.770067) /
          4.453725) *
          10,
        SPEC == 410 &
          MISSION == "NED2010027" &
          SETNO %in% c(219, 222, 223, 229, 230, 233, 236) ~ FLEN * 10,
        SPEC == 410 &
          MISSION == "NED2019102" &
          SETNO %in% c(13, 15, 132, 133, 150) ~ FLEN * 10,
        SPEC == 410 &
          MISSION == "NED2019030" &
          SETNO %in% c(132, 133, 150) ~ FLEN * 10,
        TRUE ~ FLEN
      )
    )
}
...
valPerSqKm <- function(theData = NULL, towDist_NM = 1.75, netWidth_ft = 41) {
  ...
  nmi2mi = 1.1507794
  mi2ft = 5280
  sakm2 = (netWidth_ft * ft2m * m2km) *
    (towDist_NM * nmi2mi * mi2ft * ft2m * m2km)
  res = theData / sakm2
  res = round(res, 4)
  return(res)
}
...
#' @author Mike McMahon, \email{Mike.McMahon@@dfo-mpo.gc.ca}
#' @export
calcTotalSE_stratified <- function(
  theDataByStrat = NULL,
  valueField = NULL,
  areaField = NULL
) {
  ...
  res <- round(
    sqrt(sum(
      (theDataByStrat[[areaField]] / totArea)^2 * theDataByStrat[[valueField]]^2
    )),
    5
  )
  return(res)
...
calcTotalMean <- function(
  ...
  totArea <- sum(theDataByStrat[[areaField]])
  res <- round(
    sum(theDataByStrat[[valueField]] * theDataByStrat[[areaField]] / totArea),
    5
  )
  return(res)
}
...
standardize_catch_counts <- function(tblList, towDist = 1.75, by_sex = FALSE) {
  if (!"SPEC" %in% names(tblList$GSCAT) | nrow(tblList$GSDET) < 1) {
    stop(
      "Either this is Taxa-level data or GSDET has no records.  Either way, please use stratify_simple() instead"
    )
  }
  weight_data <- tblList$GSCAT |>
    dplyr::select(MISSION, SETNO, SPEC, SIZE_CLASS, SAMPWGT, TOTWGT)

  dist_data <- tblList$GSINF |>
    dplyr::select(MISSION, SETNO, DIST, AREA_KM2, GEAR, WINGSPREAD_FT)

  all_sets <- tblList$GSINF |>
    dplyr::filter(TYPE == 1) |>
    dplyr::select(MISSION, STRAT, SETNO, AREA_KM2) |>
    dplyr::distinct()
  ...
  standardized_data <- tblList$GSDET |>
    ...
    dplyr::left_join(dist_data, by = c("MISSION", "SETNO")) |>
    dplyr::mutate(
      weight_ratio = dplyr::case_when(
        is.na(TOTWGT) | is.na(SAMPWGT) | TOTWGT == 0 | SAMPWGT == 0 ~ 1,
        TRUE ~ TOTWGT / SAMPWGT
      ),
      FSEX = dplyr::if_else(is.na(FSEX), 0, dplyr::if_else(FSEX == 3, 2, FSEX)),
      CLEN = CLEN * weight_ratio,
      CAGE = dplyr::if_else(!is.na(AGE), CLEN, NA_real_),
      DIST = dplyr::if_else(is.na(DIST), towDist, DIST),
      CLEN_RAW = CLEN,
      CAGE_RAW = CAGE,
      CLEN = CLEN * (towDist / DIST),
      CAGE = dplyr::if_else(!is.na(CAGE), CAGE * (towDist / DIST), NA_real_),
      CLEN_sqkm = dplyr::if_else(
        !is.na(CLEN),
        valPerSqKm(CLEN, towDist_NM = DIST, netWidth_ft = WINGSPREAD_FT),
        NA_real_
      ),
      CAGE_sqkm = dplyr::if_else(
        !is.na(CAGE),
        valPerSqKm(CAGE, towDist_NM = DIST, netWidth_ft = WINGSPREAD_FT),
        NA_real_
      )
    ) |>
    dplyr::select(-weight_ratio, -SAMPWGT, -TOTWGT, -SIZE_CLASS)
  ...
  length_total <- if (any(!is.na(standardized_data$FLEN))) {
    length_data <- standardized_data |>
      dplyr::filter(!is.na(FLEN)) |>
      dplyr::summarise(
        CLEN_TOTAL = sum(CLEN, na.rm = TRUE),
        CLEN_SQKM_TOTAL = sum(CLEN_sqkm, na.rm = TRUE),
        .by = dplyr::all_of(base_groups) |> c("FLEN")
      )
    if (by_sex) {
      all_combos <- all_sets |>
        dplyr::cross_join(spec_only) |>
        dplyr::cross_join(
      ...
      all_combos |>
        dplyr::left_join(
          length_data,
          by = c("MISSION", "SETNO", "SPEC", "FLEN", "FSEX")
        ) |>
        dplyr::mutate(
  ...
  age_total <- if (any(!is.na(standardized_data$AGE))) {
    age_data <- standardized_data |>
      ...
      dplyr::summarise(
        ...
        .by = dplyr::all_of(base_groups) |> c("AGE")
      )
    if (by_sex) {
      all_combos <- all_sets |>
        dplyr::cross_join(spec_only) |>
        dplyr::cross_join(
          age_data |> dplyr::select(AGE) |> dplyr::distinct()
        ) |>
        dplyr::cross_join(
          length_data |> dplyr::select(FSEX) |> dplyr::distinct()
        )
      ...
      all_combos |>
        dplyr::left_join(
          age_data,
          by = c("MISSION", "SETNO", "SPEC", "AGE", "FSEX")
        ) |>
        dplyr::mutate(
      ...
      all_combos <- all_sets |>
        dplyr::cross_join(spec_only) |>
        dplyr::cross_join(age_data |> dplyr::select(AGE) |> dplyr::distinct())

      all_combos |>
        dplyr::left_join(age_data, by = c("MISSION", "SETNO", "SPEC", "AGE")) |>
        dplyr::mutate(
          CAGE_TOTAL = ifelse(is.na(CAGE_TOTAL), 0, CAGE_TOTAL),
          CAGE_SQKM_TOTAL = ifelse(is.na(CAGE_SQKM_TOTAL), 0, CAGE_SQKM_TOTAL)
        )
...
widen_data <- function(
  ...
  if (missing(var_col)) {
    stop("Parameter 'var_col' is missing.")
  }
  if (missing(value_col)) {
    stop("Parameter 'value_col' is missing.")
  }
  ...
  if (var_col == "FLEN") {
    ...
    seq_vals <- seq(
      1 + bin_size * floor(min_val / bin_size),
      1 + bin_size * floor(max_val / bin_size),
      by = bin_size
    )
  } else {
  ...
  fill_list <- stats::setNames(list(0), value_col)
  result <- data |>
    dplyr::summarise(
      "{value_col}" := sum(.data[[value_col]]),
      .by = dplyr::all_of(group_vars)
    ) |>
    tidyr::complete(
      tidyr::nesting(!!!rlang::syms(nesting_vars)),
    ...
    dplyr::arrange(dplyr::across(dplyr::all_of(arrange_vars)))
...
summarize_by_age2 <- function(
  ...
  result <- sapply(ages, function(a) {
    ...
    strata <- unique(subset[[group_col]])
    if (method == "weighted_mean") {
      mean_by_stratum <- sapply(strata, function(s) {
        mean(subset[subset[[group_col]] == s, value_col], na.rm = TRUE)
      })
      area_by_stratum <- as.numeric(sapply(strata, function(s) {
        unique(subset[subset[[group_col]] == s, area_col])[1]
      }))
      calcTotalMean(
        theDataByStrat = data.frame(
          value = mean_by_stratum,
          area = area_by_stratum
        ),
        valueField = "value",
        areaField = "area"
      )
    } else if (method == "rss_se") {
      first_by_stratum <- as.numeric(sapply(strata, function(s) {
        subset[subset[[group_col]] == s, value_col][1]
      }))
      area_by_stratum <- as.numeric(sapply(strata, function(s) {
        unique(subset[subset[[group_col]] == s, area_col])[1]
      }))
      calcTotalSE_stratified(
        theDataByStrat = data.frame(
          value = first_by_stratum,
          area = area_by_stratum
        ),
        valueField = "value",
        areaField = "area"
      ...
      calcTotalSE_unstratified(
        theDataByStrat = subset[subset[[value_col]] != 0, ],
