...
extractOpenData <- function(
  ...
  this$GSINF[!is.na(this$GSINF$DEPTH), "DEPTH_M"] <- this$GSINF[
    !is.na(this$GSINF$DEPTH),
    "DEPTH"
  ] #overwrite w depth, where avail
  ...
  utils::write.csv(
    this$GSMISSIONS,
    file = file.path(path, paste0(fn, "_GSMISSIONS.csv")),
    row.names = F
  )
  utils::write.csv(
    this$GSINF,
    file = file.path(path, paste0(fn, "_GSINF.csv")),
    row.names = F
  )
  utils::write.csv(
    this$GSCAT,
    file = file.path(path, paste0(fn, "_GSCAT.csv")),
    row.names = F
  )
  if ("GSCAT_CONV" %in% names(this)) {
    utils::write.csv(
      this$GSCAT_CONV,
      file = file.path(path, paste0(fn, "_GSCAT_CONV.csv")),
      row.names = F
    )
  }
  utils::write.csv(
    this$GSDET,
    file = file.path(path, paste0(fn, "_GSDET.csv")),
    row.names = F
  )
  if ("GSDET_CONV" %in% names(this)) {
    utils::write.csv(
      this$GSDET_CONV,
      file = file.path(path, paste0(fn, "_GSDET_CONV.csv")),
      row.names = F
    )
  }
  utils::write.csv(
    this$GSSPECIES_NEW,
    file = file.path(path, paste0(fn, "_GSSPECIES.csv")),
    row.names = F
  )
  return(this)
}
