...
stratify <- function(tblList=NULL, df=NULL, towDist_NM = 1.75, areaField = "AREA", areaFieldUnits= c("NM2","KM2"),debug=F){
  ...
  overall <- data.frame(
    ...
    BIOMASS = BIOMASS_OVERALL$value,
    BIOMASS_SE = BIOMASS_OVERALL$se,
    BIOMASS_LOW = BIOMASS_OVERALL$low,
    BIOMASS_HIGH = BIOMASS_OVERALL$high,
    ABUNDANCE = ABUNDANCE_OVERALL$value,
    ABUNDANCE_SE = ABUNDANCE_OVERALL$se,
    ABUNDANCE_LOW = ABUNDANCE_OVERALL$low,
    ABUNDANCE_HIGH = ABUNDANCE_OVERALL$high
  )
